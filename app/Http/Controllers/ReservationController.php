...
class ReservationController extends Controller
{
    ...
    {
        // Setelah memperbarui status reservasi, dapatkan reservasi dengan status "Done" (0)
        $reservations = Reservation::where('status', 0)->get();

        // Buat data yang akan dikirim sebagai response SSE
        $data = [];

        foreach ($reservations as $reservation) {
            $data[] = [
                'reservation_id' => $reservation->reservation_id,
                'message' => 'Reservation ID ' . $reservation->reservation_id . ' telah selesai.',
            ];
        }
        ...
        // Tutup koneksi
        fclose(ob_get_status()['@ob.buffer']);
        die();
    }
